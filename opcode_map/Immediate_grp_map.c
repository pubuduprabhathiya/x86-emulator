...
map *immediate_opcode_map_group_1[]={immediate_opcode_map_80,immediate_opcode_map_81,immediate_opcode_map_82,immediate_opcode_map_83};
...
struct group get_group(unsigned char byte){
    ...
    if(byte==0X80 || byte==0X81  ||byte==0X82 ||byte==0X83 ) {
        grp.group=1;
        if(byte==0X80 )grp.index=0;
        if(byte==0X81 )grp.index=1;
        if(byte==0X82 )grp.index=2;
        if(byte==0X83 )grp.index=3;
        return grp;
    };
    return grp;
}
